    ...
    if($_SESSION['id']=="") header("Location:../../../");
     ...
     if (mysqli_num_rows($result) > 0) {
            ...
            if (mysqli_query($conn, $sql)) {
                header('Location: ../core/ae_daily_update.php?month='.number_format(date_format($date,"m")));
            } else {
            ...
            VALUES ('".date_format($date,"Y-m-d")."', '".date_format($date,"d")."','".date_format($date,"m")."','".date_format($date,"Y")."','".$_POST["no_rooms_occupied"]."', '".$_POST["no_stayed_overnight"]."', '".$_POST["no_new_checkin"]."','".$_POST["no_male"]."', '".$_POST["no_female"]."', '".$_POST["local_tourist"]."',
              '".$_POST["foreign_tourist"]."', '".$_POST["overseas_filipino"]."', '".$_POST["local_details"]."','".$_POST["foreign_details"]."','".$_POST["overseas_details"]."','".addslashes($_POST["remarks"])."','".date("Y-m-d | H:i:s")."','".$_POST["ae_id"]."', '".$encoder."')";
            if (mysqli_query($conn, $sql)) {
                if($_SESSION['level']>2){
