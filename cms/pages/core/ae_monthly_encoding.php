<?php
  $page = "ae_monthly_encoding";
  include '../template/header.php';
  recordLog("Visited AE Monthly Encoding");
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>AE Data Encoding </h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">AE Data Encoding </li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <div class="card-body">
        <form class="form-row" action="../crud/sync_ae_data.php" method="GET">
            <!-- Province Select -->
            ...
            <div class="form-group col-md-3">
                ...
                <select name="citymun" id="citymunData" class="form-control" 
                    ...
                    onchange="showAccommodations(this.value)" required>
            ...
            <!-- Accommodation Establishments Select -->
            <div class="form-group col-md-4">
                <label for="accommodationData">Accommodation:</label>
                <select name="ae_user" id="accommodationData" class="form-control" required>
                    <option value="">Select an Establishment</option>
                    <?php
                        $sql = "SELECT u.user_id,ae_id, ae_name FROM ts_users u
                                LEFT JOIN accommodation_establishment a ON u.user_id = a.user_id
                                WHERE a.approve_status = '1' AND u.province_c = '".$province."' AND u.citymun_c = '".$citymun."'";
                        $result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
                            while($row = mysqli_fetch_assoc($result)) {
                                echo '<option value="'.$row["user_id"].'">'.$row["ae_name"].'</option>';
                            }
                        }
                    ?>
                </select>
            ...
            <div class="form-group col-12 text-center mt-3">
                <button type="submit" class="btn btn-success">Sync Establishment Data</button>
            </div>
        </form>
                ...
                <h4 class="box-title">
                  ...
                  include '../../../connection/connection.php';
                  $totalrooms = 0;
                  $ae_name = "- No Selected Establishment Yet -";
                  $sql1 = "select ae_name, no_rooms from accommodation_establishment where user_id = '".$_SESSION['ae_user']."'";
              ...
              <table class="table table-striped" style="text-align: center;">
                  ...
                  from ae_daily_task where user_id = '".$_SESSION['ae_user']."' and month='".$x."' and year = '".date("Y")."' group by month order by month asc";

                  if($_GET["year"]!=""){
                    $sql = "select month, year, count(*) as num, sum(no_rooms_occupied), sum(no_stayed_overnight), sum(no_male), sum(no_female), sum(local_tourist), sum(foreign_tourist)
                  from ae_daily_task where user_id = '".$_SESSION['ae_user']."' and month='".$x."' and year = '".$_GET['year']."' group by month order by month asc";
                  }
                  $result = mysqli_query($conn, $sql);
                        ...
                        while($row = mysqli_fetch_assoc($result)) {
                              ...
                              echo
                                    '<tr>
                                          ...
                                          <td>
                                          <a href="ae_daily_encoding.php?month='.$row["month"].'&year='.$year.'" title="Update"> <button class="btn btn-warning btn-xs"><i class="fa fa-edit"></i></button></a>
                                          </td>
                                    </tr>';
                                }
                              }
                              ...
                              if($x<=date("m") || ($_GET["year"]!="" && $_GET["year"]< date("Y"))){
                                ...
                                if($_GET["year"]!=""){
                                  ...
                                  echo "<a href='ae_daily_encoding.php?month=".$x.'&year='.$year."' title='Add Data'> <button class='btn btn-success btn-xs'><i class='fa fa-plus'></i></button></a>";
                              }
...
<script>
    ...
    function showAccommodations(citymun_c){
        var province_c = document.getElementById("province").value;
        $.ajax({
          type: "GET",
          url: "../crud/loadAEs.php",
          cache: false,
          data: {province_c, citymun_c},
          success: function(data){
            $("#accommodationData").html(data);
          }
        });
    }
