...
?>
  <thead>
    <th colspan="14" style="color: red;">TA REGIONAL SUMMARY REPORT FOR CY <?php echo $_GET['year']; ?></th>
  </thead>
  <thead>
  ...
  <?php
      ...
      $sql = "select province_c, province_m from province where region_c = '".$region."'";
        ...
        $result = mysqli_query($conn, $sql);
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_assoc($result)) {
                echo "<tr><td><a href='#' onclick='generateReport2(3,".$year.",".($row['province_c']*1).",0)'>".$row['province_m']."</a></td>";

                   $sql1 = "select p.province_c, province_m, sum(r_male) as rm, sum(r_female) as rf, sum(nr_male) as nrm, sum(nr_female) as nrf, sum(fo_male) as fm, sum(fo_female) as ff, d.month  from province p left join ts_users t on p.province_c = t.province_c left join tourist_attraction a on t.user_id = a.user_id left join  ta_daily_task d on a.ta_id = d.ta_id where a.approve_status = '1'  and d.year = '".$year."' and p.province_c = '".$row['province_c']."' group by d.month,province_m";
