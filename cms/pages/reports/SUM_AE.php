        ...
        $result = mysqli_query($conn, $sql);
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_assoc($result)) {
                ...
                ";
                   $sql1 = "select sum(local_tourist) as domestic, sum(foreign_tourist) as foreigns, sum(overseas_filipino) as overseas_filipino, sum(no_male) as male, sum(no_female) as female from ts_users u left join accommodation_establishment a  on u.user_id = a.user_id left join ae_daily_task z on a.ae_id = z.ae_id where year = '".$year."' and approve_status = '1' and z.ae_id='".$row['ae_id']."' group by z.ae_id";
